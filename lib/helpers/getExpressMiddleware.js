...
function extractMatchedRoute(routeName, path) {
    var routeArray = !Array.isArray(routeName) ? [routeName] : routeName;
    for (index = 0; index < routeArray.length; ++index) {
        // is it a regex? find match and return
        if (routeArray[index] instanceof RegExp) {
            return routeArray[index].exec(path)[0];
        }
        // just simply match
        if (routeArray[index] === path) {
            return routeArray[index];
        }
    }

    // In case we didn't find anything at least return the first route
    // TODO: support parameterized route definitions in arrays
    return routeArray[0];
}
...
function findRouteName(req, res) {
    ...
    if (req.route && req.route.path) {
        var routeName = extractMatchedRoute(req.route.path, req.path);
