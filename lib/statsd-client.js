...
function StatsDClient(options) {
    ...
    if (this.options.formatter) {
        if (typeof this.options.formatter === 'string') {
            this._formatter = require('./format/' + this.options.formatter);
        } else {
            this._formatter = this.options.formatter;
        }
    } else {
        this._formatter = require('./format/datadog');
    }
...
StatsDClient.prototype.gauge = function gauge(name, value, tags) {
    var data = this._formatter.format(
...
StatsDClient.prototype.gaugeDelta = function gaugeDelta(name, delta, tags) {
    var value = (delta >= 0 ? "+" : "-") + Math.abs(delta);
    var data = this._formatter.format(
        this.options.prefix + name,
        value,
        'gauge',
 ...
 */
StatsDClient.prototype._mergeTags = function _mergeTags(metric_tags) {
    var tags = {};
...
StatsDClient.prototype.formatTags = function formatTags(metric_tags) {
    var tags = this._mergeTags(metric_tags);
