...
function StatsDClient(options) {
    ...
    // Ephemeral socket
    this._ephemeralSocket = new EphemeralSocket(options);
}
...
StatsDClient.prototype.gauge = function (name, value) {
    this._ephemeralSocket.send(name + ":" + value + "|g");
};
...
StatsDClient.prototype.counter = function (name, delta) {
    this._ephemeralSocket.send(name + ":" + delta + "|c");
};
...
StatsDClient.prototype.timing = function (name, time) {
    ...
    this._ephemeralSocket.send(name + ":" + t + "|ms");
};
