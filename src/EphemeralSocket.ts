...
import {debuglog} from 'util';
    ...
    const debug = debuglog('statsd-client');

/*global console*/

interface EphemeralSocketOptions {
    host?: string
    port?: number | string
...
class EphemeralSocket{
    ...
    constructor (options?: EphemeralSocketOptions){
        if (!options) { options = {}; }

        this._hostname = options.host || 'localhost';
        this._port = (typeof options.port === 'string' ? parseInt(options.port, 10) : options.port) || 8125;
